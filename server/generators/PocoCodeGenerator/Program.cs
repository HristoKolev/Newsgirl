        ...
        public static async Task Main()
        {
            ...
            foreach (var setting in settings)
            {
                await using (var connection = new NpgsqlConnection(setting!.ConnectionString))
                {
                    ...
                    var render = new RazorRenderer<PocoTemplateContext>(templateString, new[] {typeof(Enumerable).Assembly});
        ...
        public async Task<List<FunctionMetadataModel>> GetFunctions()
        {
            var functions = await Execute<FunctionMetadataModel>(this.Connection, @"
                 SELECT
                    ...
                    (case pg_get_function_identity_arguments(f.oid) when '' then null else pg_get_function_identity_arguments(f.oid) end)
                        as FunctionArgumentsAsString,

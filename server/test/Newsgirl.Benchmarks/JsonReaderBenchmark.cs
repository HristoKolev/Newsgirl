    ...
    public class JsonReaderBenchmark
    {
        ...
        public void JsonSerializer_DeserializeAsync()
        {
            for (int i = 0; i < this.N; i++)
            {
                ...
                System.Text.Json.JsonSerializer.DeserializeAsync<MyModel>(this.stream).GetAwaiter().GetResult();
            }
        }
        ...
        public void JsonDocPlusDeserialize()
        {
            for (int i = 0; i < this.N; i++)
            {
                ...
                var model = System.Text.Json.JsonSerializer.Deserialize<MyModel>(buffer.AsSpan(0, offset));
        ...
        public void JsonSerializer_Deserialize_From_Reader()
        {
            for (int i = 0; i < this.N; i++)
            {
                ...
                var model = System.Text.Json.JsonSerializer.Deserialize<MyModel>(ref reader);
        ...
        public void JsonNet_Deserialize()
        {
            for (int i = 0; i < this.N; i++)
            {
                ...
                var model = JsonConvert.DeserializeObject<MyModel>(str);
